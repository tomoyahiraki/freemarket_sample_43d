...
# before_action :move_to_index, except: :index
  ...
  def signinphonenumber
    # binding.pry
    @users = Usertest.new
    session[:nickname] = params[:usertest][:nickname]
    session[:email] = params[:usertest][:email]
    session[:password] = params[:usertest][:password]
    session[:first_name] = params[:usertest][:first_name]
    session[:last_name] = params[:usertest][:last_name]
    session[:kana_first_name] = params[:usertest][:kana_first_name]
    session[:kana_last_name] = params[:usertest][:kana_last_name]
    session[:birth_year_id] = params[:usertest][:birth_year_id]
    session[:birth_month_id] = params[:usertest][:birth_month_id]
    session[:birth_day_id] = params[:usertest][:birth_day_id]
  end

  def signinlocation
    # binding.pry
    @user_address = UserAddress.new
    session[:telphone_number] = params[:usertest][:telphone_number]
  end
